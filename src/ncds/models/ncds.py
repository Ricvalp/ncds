...
class NCDS:
    ...
    def create_functions(self):
        ...
        def losses(params, batch, dt):
            ...
            pred = jax.vmap(self.model.apply, in_axes=(None, 0))(
                params, batch[:, :-1]
            ).squeeze(-1)

            # pred = self.model.apply(params, batch[:, :-1])

            velocity_loss = jnp.mean(jnp.sum((pred - velocity) ** 2, axis=-1))
    ...
    def init_model(self, init_example: Array) -> Any:
        ...
        self.state = TrainState.create(
            apply_fn=self.model.apply, params=model_params, tx=tx, weights=weights
        )
...
class J_net(nn.Module):
    ...
    def setup(self):
        ...
        self.epsilon = (
            ...
            else self.param("epsilon", lambda _: self.epsilon_init)
        )

    def __call__(self, x: Array) -> Array:
        g_out = self.g_net(x)
        j_out = -(g_out @ jnp.transpose(g_out, (0, 2, 1))) + self.epsilon * jnp.eye(
            g_out.shape[-1]
        )
        return j_out
...
class F_net(nn.Module):
    ...
    def setup(self):
        ...
        self.x0 = (
            self.x0_init
            if not self.train_x0
            else self.param("x0", lambda _: self.x0_init)
        )
